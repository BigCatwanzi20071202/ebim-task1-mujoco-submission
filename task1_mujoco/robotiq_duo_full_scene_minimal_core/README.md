...
Mobile dual-FR3 + Robotiq 2F-85 cable routing in the full board / fixture /
room scene. One entry point for teleoperation and the stage-1 autonomous
O-prop/C-prop baselines:
...
conda run -n mujoco python main.py --input baseline     # one O-prop closed loop
conda run -n mujoco python main.py --input baseline_c   # one C-prop closed loop
```
...
The autonomous baseline uses MuJoCo truth for stage-1 localization, then
reuses the shared base, DLS IK, contact clamp and force-servo grasp stack. A
headless run executes the complete primitive rather than a short smoke test:

```bash
python main.py --input baseline --no-viewer
python main.py --input baseline --help
python main.py --input baseline_c --no-viewer
python main.py --input baseline_c --help
```

It is one development primitive, not the complete ordered Task 1 and not an
MNet completion report. See [`../TASK1_MUJOCO_PLAN.md`](../TASK1_MUJOCO_PLAN.md)
for the state machine, verification criteria, official constraints and staged
implementation plan.
...
- `teleop/` — all code, one module per concern (module map in
  ...
  - `o_prop_baseline.py`, `c_prop_baseline.py`, `run_baseline.py`,
    `run_c_baseline.py` — single-fixture autonomous state machines, geometry
    verification, safety checks and runtime

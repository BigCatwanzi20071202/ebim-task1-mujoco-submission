...
def add_viewer_args(parser: argparse.ArgumentParser) -> None:
    ...
    g.add_argument(
        "--no-viewer",
        action="store_true",
        help="run without a GUI (desktop/VR use a short smoke test; baseline runs to completion)",
    )
...
def add_physics_args(
    parser: argparse.ArgumentParser,
    *,
    timestep_default: float,
    noslip_default: int | None = None,
) -> None:
    ...
    g.add_argument(
        "--noslip-iterations",
        type=int,
        default=noslip_default,
        help=f"override XML noslip_iterations (default "
        f"{'XML=20' if noslip_default is None else noslip_default}; 0 skips the noslip pass, "
...
def add_baseline_args(parser: argparse.ArgumentParser) -> None:
    g = parser.add_argument_group("stage-1 autonomous O-prop baseline")
    g.add_argument(
        "--baseline-prop",
        choices=tuple(f"round_peg_{i}" for i in range(4)),
        default=config.BASELINE_O_PROP,
        help="single round peg to route around",
    )
    g.add_argument(
        "--baseline-direction",
        choices=("ccw", "cw"),
        default="ccw",
        help="required cable routing direction, viewed from above",
    )
    g.add_argument(
        "--baseline-timeout",
        type=float,
        default=config.BASELINE_TOTAL_TIMEOUT,
        help="maximum simulated seconds before a safe stop",
    )
    g.add_argument(
        "--randomize-board",
        action="store_true",
        help="randomize Tier-2 fixtures before locating the cable and prop",
    )
    g.add_argument("--randomize-seed", type=int, default=None)
    g.add_argument(
        "--baseline-props",
        type=str,
        default=None,
        help="comma-separated list of sequential round peg names to route around; overrides --baseline-prop",
    )
    g.add_argument(
        "--baseline-repeats",
        type=int,
        default=1,
        help="repeat the same baseline operation this many times when no prop list is provided",
    )


def add_c_baseline_args(parser: argparse.ArgumentParser) -> None:
    g = parser.add_argument_group("stage-1 autonomous C-prop baseline")
    g.add_argument(
        "--baseline-timeout",
        type=float,
        default=config.BASELINE_TOTAL_TIMEOUT,
    ...
    g.add_argument(
        ...
        type=str,
        default=None,
        help="comma-separated list of sequential C-clip names to route through; overrides the single C-clip default",
    )
    g.add_argument(
        "--baseline-repeats",
        type=int,
        default=1,
        help="repeat the same C-prop primitive this many times sequentially",
    )
    g.add_argument(
        "--randomize-board",
        action="store_true",
        help="randomize Tier-2 fixtures before locating the cable and C-prop",
    )
    g.add_argument("--randomize-seed", type=int, default=None)


def add_oc_baseline_args(parser: argparse.ArgumentParser) -> None:
    add_baseline_args(parser)
    g = parser.add_argument_group("stage-1 O-prop then C-prop sequence")
    g.add_argument(
        "--c-baseline-prop",
        choices=(config.C_BASELINE_CLIP,),
        default=config.C_BASELINE_CLIP,
        help="C-clip to route through after the O-prop succeeds",
    )


def add_ocy_baseline_args(parser: argparse.ArgumentParser) -> None:
    add_oc_baseline_args(parser)
    g = parser.add_argument_group("stage-1 O, C, then Y sequence")
    g.add_argument(
        "--y-baseline-prop",
        choices=(config.Y_BASELINE_PROP,),
        default=config.Y_BASELINE_PROP,
        help="Y checkpoint to validate after O and C",
    )
...
def build_desktop_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--input",
        choices=("keyboard", "gamepad", "vr", "gello", "ros_teleop", "baseline", "baseline_c"),
        default="keyboard",
...
def build_baseline_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Duo-FR3 stage-1 autonomous baseline - grasp and route around one O-prop",
    )
    parser.add_argument(
        "--input",
        choices=("baseline",),
    ...
    add_viewer_args(parser)
    add_physics_args(parser, timestep_default=0.002, noslip_default=0)
    add_grasp_args(parser)
    add_base_args(parser, base_speed_default=1.0, base_yaw_default=90.0, with_control_modes=True)
    add_baseline_args(parser)
    return parser


def build_c_baseline_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Duo-FR3 stage-1 autonomous baseline - grasp and route through one C-prop",
    )
    parser.add_argument(
        "--input",
        choices=("baseline_c",),
    ...
    add_grasp_args(parser)
    add_base_args(parser, base_speed_default=1.0, base_yaw_default=90.0, with_control_modes=True)
    add_c_baseline_args(parser)
    parser.set_defaults(
        baseline_prop=config.C_BASELINE_CLIP,
        baseline_direction="ccw",
        base_control="actuator",
    )
    return parser


def build_oc_baseline_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Duo-FR3 stage-1 autonomous baseline - route one O-prop, then one C-prop",
    )
    parser.add_argument(
        "--input",
        choices=("baseline_oc",),
    ...
    add_base_args(
        parser,
        base_speed_default=1.0,
        base_yaw_default=90.0,
        with_control_modes=True,
    )
    add_oc_baseline_args(parser)
    return parser


def build_ocy_baseline_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Duo-FR3 stage-1 autonomous baseline - O, C, then bimanual Y checkpoint",
    )
    parser.add_argument(
        "--input",
        choices=("baseline_ocy",),
        default="baseline_ocy",
        help="input method (dispatched by main.py)",
    )
    add_viewer_args(parser)
    add_physics_args(parser, timestep_default=0.002, noslip_default=0)

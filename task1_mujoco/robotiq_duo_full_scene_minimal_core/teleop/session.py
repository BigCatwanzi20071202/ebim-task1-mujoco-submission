...
class TeleopSession:
    def __init__(self, args) -> None:
        ...
        # Spawn with the right gripper hovering over the selected grasp area.
        # The C primitive needs a segment beside the clip; other modes retain
        # the established free-end placement.
        self.start_lookat: np.ndarray | None = None
        if args.start_at_board:
            placed = False
            start_arm = self.arms[
                config.C_BASELINE_ARM if args.input == "baseline_c" else "right"
            ]
            if args.input == "baseline_c" and self.clip_body is not None:
                rotation = self.data.xmat[self.clip_body].reshape(3, 3)
                desired = (
                    self.data.xpos[self.clip_body]
                    + rotation @ config.C_BASELINE_GRASP_LOCAL
                )
                cable_points = self.data.xpos[self._cable_body_arr]
                target = cable_points[
                    int(np.argmin(np.linalg.norm(cable_points - desired, axis=1)))
                ].copy()
                placed = teleport_base_slot_to_target(
                    self.model,
                    self.data,
                    start_arm,
                    target,
                    preferred_outward=config.C_BASELINE_BASE_OUTWARD,
                )
                start_message = (
                    f"{start_arm.name} gripper over the C-prop grasp segment"
                )
            else:
                placed = teleport_base_near_cable(
                    self.model, self.data, self.arms["right"]
                )
                start_message = "right gripper over the cable free end"
            if placed:
                log(f"[start] base placed at board, {start_message}")
    ...
    def step_once(self, dt: float, *, follow_tcp_quat: bool = False) -> None:
        ...
        cable_decay = float(getattr(self, "cable_velocity_decay", 0.0))
        if cable_decay > 0.0:
            self.data.qvel[self.cable_dofs] *= np.exp(-cable_decay * dt)

...
Module map (edit only the layer you are working on):
  ...
  o_prop_baseline.py closed-loop state machine + O-prop result detector
  run_baseline.py    stage-1 autonomous baseline runtime
  c_prop_baseline.py single C-prop trajectory + through-channel detector
  run_c_baseline.py  C-prop runtime entry

...
class Arm:
    ...
    grasp_offset: np.ndarray | None = None  # held cable point relative to pad slot
    grasp_assist_age: float = 0.0
    grasp_nocontact_time: float = 0.0
    grasp_release_locked: bool = False
...
def solve_twist_ik(
    ...
    """Return the damped-least-squares joint velocity for a TCP twist."""
    ...
    if float(np.linalg.norm(twist)) < 1e-8:
        return np.zeros(len(arm.dof_ids), dtype=np.float64)
...
def apply_twist_ik(
    ...
    """Map a TCP twist to the existing FR3 velocity actuators."""
    qvel = solve_twist_ik(model, data, arm, twist)
...
def apply_twist_ik_kinematic(
    ...
    """Apply the shared IK solution as a bounded per-tick joint increment.

    This is intended for deterministic autonomous primitives at table-inner
    spawn poses where the high-gain velocity actuators become numerically
    unstable. Contacts and cable dynamics still run through ``mj_step``; only
    the arm joint servo is position-stepped, matching the existing idle hold.
    """
    # Restore the commanded anchor before computing IK. Without this, the
    # gravity drift accumulated during the preceding mj_step is integrated as
    # if it were part of the command and quickly dominates the small TCP step.
    for i, (joint_id, dof_id) in enumerate(zip(arm.joint_ids, arm.dof_ids)):
        data.qpos[model.jnt_qposadr[joint_id]] = float(arm.q_ref[i])
    ...
    for i, (joint_id, dof_id) in enumerate(zip(arm.joint_ids, arm.dof_ids)):
        ...
        arm.q_ref[i] = float(
            np.clip(arm.q_ref[i] + qvel[i] * float(dt), low, high)
        )
        data.qpos[qadr] = arm.q_ref[i]
        data.qvel[dof_id] = 0.0
    write_arm_ctrl(model, data, arm)

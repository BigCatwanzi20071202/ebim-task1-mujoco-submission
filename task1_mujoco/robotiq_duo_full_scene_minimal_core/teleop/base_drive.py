...
class BaseDriver:
    ...
    def world_xy_to_local(self, world_xy: np.ndarray) -> np.ndarray:
        """Project a world-frame XY vector into the robot command frame."""
        world_xy = np.asarray(world_xy, dtype=np.float64)[:2]
        world_x = self._world_xy(1.0, 0.0)[:2]
        world_y = self._world_xy(0.0, 1.0)[:2]
        return np.array([np.dot(world_xy, world_x), np.dot(world_xy, world_y)])

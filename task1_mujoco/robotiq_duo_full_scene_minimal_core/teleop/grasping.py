...
def release_grasp(data: mujoco.MjData, arm: Arm) -> None:
    ...
    if arm.grasp_offset is not None:
        arm.grasp_offset[:] = 0.0
    arm.grasp_assist_age = 0.0
    arm.grasp_nocontact_time = 0.0
    arm.grasp_release_locked = False
...
def update_grasp(
    ...
    if grasp_assist and arm.grasped_body is not None:
        ...
        prev = (
            arm.prev_slot_pos
            if arm.prev_slot_pos is not None
            else held_point.copy()
        )
        arm.grasp_assist_age += dt
        if arm.grasp_assist_age < config.GRASP_ASSIST_START_DELAY:
            arm.prev_slot_pos = held_point.copy()
            return
        ...
        arm_scale = float(getattr(arm, "grasp_assist_force_scale", 1.0))
        dist = apply_grasp_assist(
            data, arm.grasped_body, held_point, prev, dt,
            force_scale=ramp * arm_scale,
        )
        for neighbor in arm.grasped_neighbors or []:
            apply_grasp_assist(
                data, neighbor, held_point, prev, dt,
                force_scale=0.25 * ramp * arm_scale,
            )
        arm.prev_slot_pos = held_point.copy()
        ...
        if not arm.grasp_release_locked and (
            escaped

"""Duo-FR3 full-scene teleop — single entry point for every input method.
    ...
    python main.py --input baseline      # autonomous single O-prop primitive
    python main.py --input baseline_c    # autonomous single C-prop primitive
    python main.py --input baseline_oc   # autonomous O-prop then C-prop sequence
    python main.py --input baseline_ocy  # autonomous O, C, then bimanual Y sequence
...
def main(argv: list[str] | None = None) -> None:
    ...
    pre.add_argument(
        "--input",
        choices=("keyboard", "gamepad", "vr", "gello", "ros_teleop", "baseline", "baseline_c", "baseline_oc", "baseline_ocy"),
        default="keyboard",
    )
    ...
    if ns.input == "baseline":
        from teleop import run_baseline

        args = cli.build_baseline_parser().parse_args(rest)
        args.input = "baseline"
        run_baseline.main(args)
        return

    if ns.input == "baseline_c":
        from teleop import run_c_baseline

        args = cli.build_c_baseline_parser().parse_args(rest)
        args.input = "baseline_c"
        run_c_baseline.main(args)
        return

    if ns.input == "baseline_oc":
        from teleop import run_oc_baseline

        args = cli.build_oc_baseline_parser().parse_args(rest)
        args.input = "baseline_oc"
        run_oc_baseline.main(args)
        return

    if ns.input == "baseline_ocy":
        from teleop import run_ocy_baseline

        args = cli.build_ocy_baseline_parser().parse_args(rest)
        args.input = "baseline_ocy"
        run_ocy_baseline.main(args)
        return

...
#   ./docker-run.sh stop                     # force-remove stuck one-off runs
...
# Docker access is configured for the project user. Running the GUI launcher
# through sudo changes HOME/XDG/X11 ownership and also leaves a root-owned
# compose client attached to the terminal when the viewer wedges on exit.
if [ "${EUID:-$(id -u)}" -eq 0 ] && [ -n "${SUDO_USER:-}" ]; then
    echo "[docker-run] Do not run this launcher with sudo." >&2
    echo "[docker-run] Re-run as: ./docker-run.sh $*" >&2
    echo "[docker-run] To remove an old stuck run: ./docker-run.sh stop" >&2
    exit 2
fi
...
stop_one_off_runs() {
    local ids=()
    mapfile -t ids < <(docker ps -aq --filter 'name=^/ebim-task1-')
    if [ "${#ids[@]}" -eq 0 ]; then
        echo "[docker-run] no managed Task 1 containers found"
        return 0
    fi
    echo "[docker-run] force-removing ${#ids[@]} managed Task 1 container(s)"
    docker rm -f "${ids[@]}"
}
...
case "${1:-}" in
    ...
    stop)   stop_one_off_runs ;;
    down)   stop_one_off_runs; "${COMPOSE[@]}" down --remove-orphans ;;
    ...
    *)
        "${COMPOSE[@]}" build sim
        run_name="ebim-task1-sim-$$"
        cleanup_run() {
            trap - EXIT INT TERM HUP
            docker rm -f "$run_name" >/dev/null 2>&1 || true
        }
        trap cleanup_run EXIT INT TERM HUP
        echo "[docker-run] container=$run_name (emergency stop: ./docker-run.sh stop)"
        "${COMPOSE[@]}" run --name "$run_name" --rm sim ebim python3 main.py "$@"
        ;;
esac

...
### Stage-1 autonomous O-prop, C-prop and Y-checkpoint work

A minimal traditional closed-loop primitive is available alongside teleop:

```bash
./start.sh --input baseline
./docker-run.sh --input baseline --no-viewer
./docker-run.sh --input baseline_c            # C-prop, with viewer
./docker-run.sh --input baseline_c --no-viewer
./docker-run.sh --input baseline_oc --no-viewer # O-prop then C-prop
```

The O primitive plans a tangent-led cable path against one round peg's outer
boundary and verifies both directed winding and a maximum cable/prop distance.
In combined mode it retains the same grasp and light tension into C instead of
releasing after O. The C primitive
grasps near the cable's free end, approaches the clip mouth, inserts the cable,
releases and verifies a settled continuous cable crossing with material on
both sides. Both use privileged MuJoCo localization at this stage and do not
claim the full ordered Task 1 or call the MNet finished service. Design,
official constraints, tests and limitations are documented in
[`TASK1_MUJOCO_PLAN.md`](TASK1_MUJOCO_PLAN.md).

The scene also contains an explicitly labelled `yclip_0` development proxy,
because the official F3/Y fixture asset is absent from the current preview.
`teleop/y_prop_baseline.py` provides a bimanual inspection transport: the two
grippers preserve their captured relative pose, add 10 mm symmetric tension,
move the cable midpoint to the Y-region centre, then remain closed for a
five-second inspection hold. The experimental `baseline_ocy` mode
connects these to an outside-table mobile-base stance planner and guarded
bimanual state machine. Current qualification synchronously prepositions both
arms, captures both ordered material points and reaches bimanual lift. The
planner now rejects spatially close bimanual grasp pairs and uses a tighter
Y-specific base stance tolerance; geometry and planning tests pass, but the
latest O-to-C handoff and inspection transport still require full headless
qualification. It does
not claim Y completion.
...
./docker-run.sh --input baseline_c        # visual C-prop baseline
./docker-run.sh --input baseline_c --no-viewer
./docker-run.sh build                    # rebuild the image only
./docker-run.sh stop                     # emergency cleanup from a 2nd terminal
```

Run the launcher as your normal user, **not with `sudo`**. `--no-viewer`
explicitly disables visualization; omit it to open the MuJoCo viewer. If a
viewer/GLFW shutdown wedges after Ctrl+C, run `./docker-run.sh stop` in a
second terminal. The launcher gives each one-off simulator a managed
`ebim-task1-*` name and force-removes it on exit. Confirm cleanup with
`docker ps --filter name=ebim-task1-`. If a host process remains in
uninterruptible `D` state even after `docker rm -f`, the kernel is waiting on a
driver and a host reboot is the final recovery method.
...
| Flag | Meaning |
|---|---|
| `--input keyboard\|gamepad\|vr\|gello\|ros_teleop\|baseline\|baseline_c` | input/controller mode (default: keyboard) |
| `--no-viewer` | headless mode; autonomous baselines run to completion |
